...
class enoMidiController:
  ...
  controllerNameDict = {'nov_launchpad_mk2':     ['novation-launchpad-mk2c-midi', False, False, True ],
                        ...
                        'akai-apcmini-mk2-midi': ['akai-apcmini-mk2-midi',        True,  True,  False],
                        'akaiApcMiniMk2':        ['akai-apcmini-mk2-midi',        True,  True,  False]}
  ...
  def startMidi(self):
    ...
    if self.isActiveDevice('aka_apcmini2') or \
       self.isActiveDevice('akaiApcMiniMk2'): self.initAkaiApcMiniMk2()
  ...
  def processMidiUpdate(self, midiStatus, midiNum, val=None):
    if self.controllerNumDict == None: 
      print("enoMidiController: processMidiUpdate called, but no registered data (%i %i %i)" % 
          (midiStatus, midiNum, val))
      return None

    midiStatNumKey = self.midiStatusNumKey(midiStatus, midiNum)
    #midiStatNumKey = '144_' + str(midiStatus) #hack; perhaps re novation?
