...
package require pdf::physicals:slot 1.0
...
########################## copy Seq Rack to Target #####################

proc copySeqRackToTarget {} {

  puts "copySeqRackToTarget"
  ...
  foreach stack $stacklist { 
    ...
    if {[itcl_info objects $stack] != {}} {
      lappend cstacklist $stack
    }
  }

  set sortedlist [lsort -command stacksorter_stub $cstacklist]

  set cumdelay 0

  puts "copying stacklist <$sortedlist>"
  ...
  foreach stack $sortedlist {
    ...
    foreach pane $paneNames {
      ...
      lappend inv_event_list \
	 "destStack receivePane $pane \{$abspos\} \{$stackrot\}"
  ...
  set n [llength $inv_event_list]
  incr n -1

  for {set i $n} {$i >= 0} {incr i -1} {
    set event [lindex $inv_event_list $i]

    puts "appending event $i = $event"

    after $cumdelay $event
...
proc pwall {which id} {
    ...
    if {!$wallup} {
	perspWall wall -basePos $wallpos -baseRot "0 0 0" -textureCache $cache -animManager [animgr getThis] -paneXdim 7.0 -paneYdim 5.3 -xlen 7.13
	set wallup 1
	...
	set cachedFiles [mapLocalTextures $texlist]
	
	#foreach tex $texlist {}

	foreach tex $cachedFiles {
...
#	puts "upper"
	if {$which == "exit"} {
	    set name [lindex $lowerStackNames $pos]
   	    puts "(working through exit: $name)" 
		...
		set id [$name get id]
		puts "(working through exit: $id)" 

		upvar #0 GLOBALeatIDdeath eatIDdeath
		if {[info exists eatIDdeath($id)] &&
		     $eatIDdeath($id) > 0} {

		     incr eatIDdeath($id) -1
                    #puts "*> $id DEC id death : $eatIDdeath($id)"
		    #puts "exit: id death of $id eaten ($eatIDdeath($id))"
		     return
		}
...
#		$name retractIv; $name delete;
		...
		if {$idx != -1} { 
		  set stacklist [lreplace $stacklist $idx $idx] 
		}
	    ...
	    upvar #0 GLOBALeatIDdeath eatIDdeath ;#argh; this is terrible!

	    upvar #0 GLOBALid2name id2name ;#gasp; this is really terrible!

	    if {[info exists id2name($id)]} {
	      set name $id2name($id)
	      regsub {^::} $name {} name

	      if {[itcl_info objects $name] != {}} {

		incr eatIDdeath($id)

                #puts "*> $id INCR id death : $eatIDdeath($id)"
	    ...
	    set eatIDdeath($id) 0
            #puts "*> $id ZERO id death : $eatIDdeath($id)"
	    ...
	    if {![info exists id2name($id)]} {
	     puts "weirdness with id2name..."
	    }

	    #rotNObj [$name getRotNode] {0 70 0}
	    #$name sset rotation_ {0 70 0}

	    rotNObj [$name getRotNode] {0 50 0}
	    $name sset rotation_ {0 50 0}
...
proc stuffMBContents {mbClient id fileList {blockLabel {}}} {

## will stuff a mediaBlock data struct with the appropriate files.
## intended particularly for function of target pad, with source of
## stackvis

  upvar #0 GLOBcache2orig cache2orig
  set dataFiles {}

  foreach file $fileList {
    if {[info exists cache2orig($file)]} {
      set file $cache2orig($file)
    }

    lappend dataFiles $file
  }

  set struct [$mbClient getMbStruct $id]

  $struct sset basePrefix  {localfile:}
  $struct sset basePostfix {}
  $struct sset mediaList   $dataFiles
  $struct sset blockLabel  $blockLabel
  $struct sset physidInst  $id

  set body [concat [$struct info class] [$struct getDbaseFields]]
  puts "body: $body"

  $mbClient putMbStruct $id $body
}

####################### move Stack ########################

proc moveStack {name pos} {
  global cache

  puts "moving stack name $name to $pos"

  if {[itcl_info objects $name] == {}} {

    puts "big oops in moveStack -- id $id name $name seems to have gone away!"
    return
  }
    
  set old_pos [$name get position_]

  placeStack $name $pos
  set new_pos [$name get position_]

  set posnode [$name getPosNode]

  puts "moving $name <$posnode> from $old_pos to $new_pos"

  animgr shiftObj $posnode $old_pos $new_pos 0.5

  $name sset stack_slot_pos $pos
}
...
proc makeStack {pos id} {
    ...
    stackVis $stackname -click_handler "global clickHandler; clickHandler $stackname" \
	    -texture_cache $cache -x_dim 5.0 -y_dim 3.8 -y_dir -1 \
	    -anim_mgr [animgr getThis] -line_width 2 \
	    -stack_slot_pos $pos -id $id
...
#top rack places
    ...
    #set portpos {19 14 9.5 5}
    set portpos {19.4 14.3 9.2 4.1}
...
proc mapLocalTextures {fileList} {

  set lowres {120x90} ;# {100x75}
  ...
  foreach file $fileList {
    ...
    if {[regexp {.mv$} $file]} { ;# movie
      set movIm [format {%s.jpg} $file]

      if {[file exists $movIm]} {
	set file $movIm; set useCached 0
      } else {continue} ;# only continue if we have a jpeg version
    }

    if {[regexp {.sb$} $file]} { ;#sboard
      puts "temporary hack replacement for sb going on..."
      set file "/u/ullmer/n/devices/images/whiteboard.sm.gif"
    }

    regsub {^.*\/(.*)\.[^\.]*$} $file {\1} basename
    set sgiCachename /extra/seqCache/$basename.sgi
    set rgbCachename /extra/seqCache/$basename.rgb

    if {![file exists $rgbCachename] || $useCached == 0} {
      puts "Converting image <$file>, saving to cache <$rgbCachename>..."
      puts "about to convert <$file / $sgiCachename>"
      exec convert $file -geometry $lowres $sgiCachename
      puts "mmph, moving on..."
      exec mv $sgiCachename $rgbCachename
    }

    lappend result $rgbCachename

    set cache2orig($rgbCachename) $orig
    set useCached 1
  }
...
stackVis destStack -texture_cache $cache -x_dim 5. -y_dim 3.8 -y_dir 1 -anim_mgr [animgr getThis] -position_ {21.5 2.2 0} -rotation_ {-30 -50 0}
...
mbpSlot targpad -host grasp -port 7703 -picPort 8
targpad activate
targpad config -entranceCB targBlockEnters -exitCB targBlockExits

proc targBlockEnters {} {
  set id [targpad getContents]

  puts "target pad: $id enters"

  destStack flowDown
}

proc targBlockExits {} {
  set id [targpad getLastContents]

  puts "target pad: $id exits"

  set exitName [destStack fade .4]

  $exitName appendExitEvent "destStack retractIv; destStack delete; createDestStack"

  set fileList [destStack get filename_list]
  puts "stuffing $id with contents \"$fileList\""

  stuffMBContents mbclient $id $fileList "Target Pad Output"

  puts "retrieving block to test"
  set struct [mbclient getMbStruct $id]
  puts ">> [$struct sself]"

  set behaviorAssoc [$struct get behaviorAssoc]

  if {[string match $id {0000024F0E0B01}]} {
    #blip printing block
    puts "Copying contents to printer..."

    blip "Copying target mediaBlock to di-laser..."

    cd ../devices
    tiAfter .1 "exec itclsh printblock.tcl $id &"
  }
}

proc createDestStack {} {
  global cache

  stackVis destStack -texture_cache $cache -x_dim 5. -y_dim 3.8 -y_dir 1 \
    -anim_mgr [animgr getThis] -position_ {21.5 2.2 0} -rotation_ {-30 -50 0}
