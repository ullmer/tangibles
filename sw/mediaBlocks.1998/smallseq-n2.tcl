...
proc destEvent {msg} {
	...
	if {$wallup} {
	    ...
	    if {$stacklist == {}} {puts "nothing in stacklist"; return}
	    set stacklen  [llength $stacklist]
	    set laststack [lindex $stacklist [expr $stacklen - 1]]
	    if {[itcl_info objects $laststack] == {}] {
	      puts "last stack has gone away; ignoring"; return
	    }

	    set paneNames [$laststack getAllPaneNames]
	    puts "sending <$paneNames> to stack $laststack

	    foreach pane $paneNames {
	      destStack receivePane $pane $wallpos "0 0 0"
	    }
	}
    } elseif {$which == 1} {
